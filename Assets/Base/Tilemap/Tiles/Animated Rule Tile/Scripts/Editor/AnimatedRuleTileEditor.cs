...
{
    [CustomEditor(typeof(AnimatedRuleTile), true)]
    [CanEditMultipleObjects]
    internal class AnimatedRuleTileEditor : Editor
    {
        ...
        private AnimatedRuleTile tile { get { return (target as AnimatedRuleTile); } }
        ...
        public void OnEnable()
        {
            if (tile.m_TilingRules == null)
                tile.m_TilingRules = new List<AnimatedRuleTile.TilingRule>();

            m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(AnimatedRuleTile.TilingRule), true, true, true, true);
        ...
        {
            AnimatedRuleTile.TilingRule rule = tile.m_TilingRules[index];
        ...
        {
            AnimatedRuleTile.TilingRule rule = new AnimatedRuleTile.TilingRule();
            rule.m_Output = AnimatedRuleTile.TilingRule.OutputSprite.Single;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            var baseFields = typeof(AnimatedRuleTile).GetFields().Select(field => field.Name);
        ...
        internal virtual void RuleOnGUI(Rect rect, int arrowIndex, int neighbor)
        {
            switch (neighbor)
            {
                case AnimatedRuleTile.TilingRule.Neighbor.DontCare:
                    break;
                case AnimatedRuleTile.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[arrowIndex]);
                    break;
                case AnimatedRuleTile.TilingRule.Neighbor.NotThis:
        ...
        internal virtual void RuleTransformOnGUI(Rect rect, AnimatedRuleTile.TilingRule.Transform ruleTransform)
        {
            switch (ruleTransform)
            {
                case AnimatedRuleTile.TilingRule.Transform.Rotated:
                    GUI.DrawTexture(rect, autoTransforms[0]);
                    break;
                case AnimatedRuleTile.TilingRule.Transform.MirrorX:
                    GUI.DrawTexture(rect, autoTransforms[1]);
                    break;
                case AnimatedRuleTile.TilingRule.Transform.MirrorY:
                    GUI.DrawTexture(rect, autoTransforms[2]);
                    break;
            }
        }

        internal void RuleNeighborUpdate(Rect rect, AnimatedRuleTile.TilingRule tilingRule, int index)
        {
        ...
        internal void RuleTransformUpdate(Rect rect, AnimatedRuleTile.TilingRule tilingRule)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
                tilingRule.m_RuleTransform = (AnimatedRuleTile.TilingRule.Transform)(int)Mathf.Repeat((int)tilingRule.m_RuleTransform + GetMouseChange(), 4);
                GUI.changed = true;
                Event.current.Use();
            }
        }
        ...
        internal virtual void RuleMatrixOnGUI(AnimatedRuleTile tile, Rect rect, AnimatedRuleTile.TilingRule tilingRule)
        {
        ...
        internal static void SpriteOnGUI(Rect rect, AnimatedRuleTile.TilingRule tilingRule)
        {
        ...
        internal static void RuleInspectorOnGUI(Rect rect, AnimatedRuleTile.TilingRule tilingRule)
        {
            float y = rect.yMin;
            EditorGUI.BeginChangeCheck();
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
            tilingRule.m_RuleTransform = (AnimatedRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
            ...
            tilingRule.m_Output = (AnimatedRuleTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == AnimatedRuleTile.TilingRule.OutputSprite.Animation)
            {
            ...
            if (tilingRule.m_Output == AnimatedRuleTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomTransform = (AnimatedRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output != AnimatedRuleTile.TilingRule.OutputSprite.Single)
            {
        ...
        {
            [SerializeField]
            public List<AnimatedRuleTile.TilingRule> rules = new List<AnimatedRuleTile.TilingRule>();
        }

        [MenuItem("CONTEXT/AnimatedRuleTile/Copy All Rules")]
        private static void CopyAllRules(MenuCommand item)
        {
            AnimatedRuleTile tile = item.context as AnimatedRuleTile;
            if (tile == null)
                return;
        ...
        [MenuItem("CONTEXT/AnimatedRuleTile/Paste Rules")]
